...
const main = (w) => {
  ...
  const [$state] = store.create(
    [
      store.state.postMetadataRecord.actions.update(
        models.postMetadataDTO
          .fromWindow(w)
          .map(models.postMetadata.fromDTO)
          .map((metadata) => [metadata.identifier, metadata])
      ),
  ...
  const $metadata = w.rxjs.combineLatest(
    $state.pipe(
      w.rxjs.map(
        lib.fn.flow(
          store.state.root.selectors.makeSelectPostMetadataRecord(),
          store.state.postMetadataRecord.selectors.makeSelectValues()
        )
      )
    ),
    $state.pipe(
  ...
  $transition
    ...
    .subscribe(([[[view, sort, order], [_, to]], [posts, tags]]) => {
      lib.maybe.tap(
        ([lxs, rx]) => {
          slots.forEach((slot) => {
            lxs.forEach((lx) => {
        ...
        lib.maybe.sequence([
          ...
          lib.element.fromString(
            (view === 'list'
              ? templates.postList.fromProps
              : templates.postGrid.fromProps)({
              tags,
              posts: posts.sort(
                lib.fn.pipe(
                  models.postMetadata.compareBy(sort),
                  models.postMetadata.orderBy(order)
                )
              ),
              id: `transition-${to}`,
            })(w)
