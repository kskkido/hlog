        ...
        rxjs.mergeMap((actor) =>
          rxjs.combineLatest([
            rxjs.combineLatest([
              ...
              components.tagFilter.views.observe(actor.tagFilter)($state),
              components.tagList.views.observe(actor.tagList)($state)(w)
            ]),
            rxjs
              .merge(
                ...
                rxjs.mergeMap(() =>
                  rxjs
                    .merge(
                      ...
                      rxjs.timer(200).pipe(rxjs.mergeMap(() => components.tagFilter.events.observe(actor.tagFilter))),
                      rxjs
                        .timer(400)
                        .pipe(rxjs.mergeMap(() => components.tagList.events.observe(actor.tagList)($state)))
                    )
                    .pipe(
                      rxjs.takeUntil(
                ...
                rxjs.mergeMap(() =>
                  rxjs
                    .combineLatest([
                      ...
                      rxjs
                        .combineLatest([
                          components.tagFilter.events.animation(actor.tagFilter)(
                            $state.pipe(
                              rxjs.map((s) => s.animations),
                              rxjs.distinctUntilChanged()
                            )
                          ),
                          components.tagFilter.events.unmount(actor.tagFilter).pipe(rxjs.tap(source[1]))
                        ])
                        .pipe(rxjs.map(Tuple.fst)),
                      rxjs
                        .combineLatest([
                          components.tagList.events.animation(actor.tagList)(
                            $state.pipe(
                              rxjs.map((s) => s.animations),
                              rxjs.distinctUntilChanged()
                            )
                          ),
                          components.tagList.events.unmount(actor.tagList).pipe(rxjs.tap(source[1]))
                        ])
                        .pipe(rxjs.map(Tuple.fst))
                    ])
                    .pipe(
