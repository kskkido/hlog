...
export const inputs = (e: lib.element.types.HTMLElement): ReadonlyArray<lib.element.types.HTMLElement> => {
  return pipe(
    ...
    ReadonlyArray.filter((e) => pipe(e.dataset.value, models.viewType.types.ViewType.is))
  );
};

export const values = (e: lib.element.types.HTMLElement): ReadonlyArray<models.viewType.types.ViewType> => {
  return pipe(
    inputs(e),
    ReadonlyArray.map((e) => pipe(e.dataset.value, Option.fromEitherK(models.viewType.types.ViewType.decode))),
    ReadonlyArray.compact
  );
};
