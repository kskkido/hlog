...
export const inputs = (e: lib.element.types.HTMLElement): ReadonlyArray<lib.element.types.HTMLElement> => {
  return pipe(
    ...
    ReadonlyArray.filter((e) => pipe(e.dataset.value, models.orderType.types.OrderType.is))
  );
};

export const values = (e: lib.element.types.HTMLElement): ReadonlyArray<models.orderType.types.OrderType> => {
  return pipe(
    inputs(e),
    ReadonlyArray.map((e) => pipe(e.dataset.value, Option.fromEitherK(models.orderType.types.OrderType.decode))),
    ReadonlyArray.compact
  );
};
