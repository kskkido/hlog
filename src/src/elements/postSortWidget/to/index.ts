...
export const inputs = (e: lib.element.types.HTMLElement): ReadonlyArray<lib.element.types.HTMLElement> => {
  return pipe(
    ...
    ReadonlyArray.filter((e) => pipe(e.dataset.value, models.postSortType.types.PostSortType.is))
  );
};

export const values = (e: lib.element.types.HTMLElement): ReadonlyArray<models.postSortType.types.PostSortType> => {
  return pipe(
    inputs(e),
    ReadonlyArray.map((e) => pipe(e.dataset.value, Option.fromEitherK(models.postSortType.types.PostSortType.decode))),
    ReadonlyArray.compact
  );
};
