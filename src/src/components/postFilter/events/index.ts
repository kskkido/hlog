...
export const view = (e: lib.element.types.HTMLElement) => {
  return rxjs
    ...
    .pipe(rxjs.map((value) => [states.postFilter.actions.set({ view: value })]));
};
...
export const sort = (e: lib.element.types.HTMLElement) => {
  return rxjs
        ...
        ReadonlyArray.map((i) =>
          rxjs.fromEvent(i, 'click').pipe(
            rxjs.map(() =>
              pipe(
                i.dataset.value,
                Option.fromEitherK(models.postSortType.types.PostSortType.decode),
                Option.toNullable
              )
            ),
            rxjs.filter((x): x is Exclude<typeof x, null> => x !== null)
    ...
    .pipe(rxjs.map((value) => [states.postFilter.actions.set({ sort: value })]));
};
...
export const order = (e: lib.element.types.HTMLElement) => {
  return rxjs
    ...
    .pipe(rxjs.map((value) => [states.postFilter.actions.set({ order: value })]));
};
